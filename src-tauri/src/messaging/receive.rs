...
pub async fn recieve(mut message: EncryptedMessage, app_handle: tauri::AppHandle)
{
    ...
    // decrypt message
    message.data = symm::decrypt(cipher, &decrypted_convo_key, None, &message.data).unwrap();
    ...
    println!("{:#?}", account.conversations.iter().find(|x| x.id == message.dest_convo_id).unwrap().messages.len());
